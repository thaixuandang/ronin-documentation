...
* [Swap](./guides/swap-tokens.mdx) between pairs of various ERC20 tokens, including wrapping RON to wRON to make offers on Mavis Market and participate in Ronin token launches, and swapping between "main" and "seed" tokens. Dynamic fees are applied based on the token pair.
* [Create liquidity pools](./guides/create-pool.mdx) with token pairs to earn trading fees, allocating liquidity within specific price ranges. Earn higher returns on your assets by focusing your liquidity where it's needed most.
...
Some features are only available in the Ronin app, while others exist in Ronin Wallet mobile app and browser extension:

|Feature|Ronin app|Ronin Wallet|
|---|---|---|
